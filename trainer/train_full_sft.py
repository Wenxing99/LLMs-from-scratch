...
from model.model import FeiFeiMindConfig  # 模型配置
...
def train_epoch(
    ...
    Args:
        ...
        loader: 数据加载器
        iters: 该epoch总迭代次数
        total_update_steps: 本次训练总共会执行多少次 optimizer 更新
        start_step: 起始步数（用于断点续训）
        start_update_step: 已经完成的 optimizer 更新步数
        wandb: 实验跟踪系统
    """
    ...
    for step, (input_ids, labels, attention_mask) in enumerate(
        ...
        if dist.is_initialized() and isinstance(model, DistributedDataParallel) and not should_update:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="FeiFeiMind Full SFT")
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        default=os.path.join(PROJECT_ROOT, "dataset", "sft_t2t_feifeimind.jsonl"),
        help="训练数据路径",
    )
    ...
    parser.add_argument("--wandb_project", type=str, default="FeiFeiMind-Full-SFT", help="wandb项目名")
    ...
    if args.from_resume == 1 and ckp_data is None:
        raise FileNotFoundError(
            f"未找到 resume checkpoint: {CHECKPOINT_DIR}/{args.save_weight}_{lm_config.hidden_size}.pth"
        )
    ...
    if ckp_data:
        ...
        start_update_step = ckp_data.get("update_step", 0)
